...
impl Log {
    pub fn from(text: String) -> Log {
        let tid_start = text.find("Tid#").expect(format!("Bad formatted Visibroker Log message missing Thread ID {}", text).as_str());
        let time_start = text.find("Tim#").expect("Badly formatted Visibroker Log message missing Time of log");
        let logger_start = text.find("Log#").expect("Bad formatted Visibroker Log message missing logger");
        let component_start = text.find("Src#").expect("Badly formatted Visibroker Log message Component missing");
        let file_start = text.find("Fil#").expect("Badly formatted Visibroker log message file name is missing");
        let line_start = text.find("Lin#").expect("Badly formatted Visibroker log message missing line");
        let level_start = text.find("Lvl#").expect("Badly formatted Visibrokr log message missing log level");
        let message_start = text.find("Msg#").expect("Badly formatted Visibroker Log message missing message");

        let pid_str: String = text.chars().skip(4).take(time_start - 4).collect();
        let time_str: String = text.chars().skip(time_start+4).take(tid_start - (time_start + 4)).collect();
        let tid_str: String = text.chars().skip(tid_start+4).take(logger_start - (tid_start+4)).collect();
        let logger_str: String = text.chars().skip(logger_start +4).take(component_start - (logger_start + 4)).collect();
        let component_str: String = text.chars().skip(component_start + 4).take(file_start - (component_start + 4)).collect();
        let file_str: String = text.chars().skip(file_start + 4).take(line_start - (file_start + 4)).collect();
        let line_str: String = text.chars().skip(line_start + 4).take(level_start - (line_start + 4)).collect();
        let level_str: String = text.chars().skip(level_start + 4).take(message_start - (level_start + 4)).collect();
        let message_str: String = text.chars().skip(message_start + 4).take(text.len() - (message_start + 4)).collect();

        return Log {
            pid: pid_str.trim().parse().expect("Unable to parse pid"),
            time: NaiveDateTime::parse_from_str(time_str.trim(),"%a %b %e %H:%M:%S %Y %fus").expect("Failed to parse date"),
            ...
            line: line_str.trim().parse().expect("Unabled to parse line number"),
...
fn main() {
    ...
    for file in args.files {
        ...
        for index in 0.. pids.len() - 1 {
            ...
            let log_str: String = contents.chars().skip(starting_pos).take(ending_pos - starting_pos).collect();
        ...
        if let Some(logger) = args.logger.clone() {
            logs = logs.into_iter().filter(|log| log.logger == logger).collect();
        }

        if let Some(component) = args.component.clone() {
            logs = logs.into_iter().filter(|log| log.component == component).collect();
        }
        ...
        if let Some(message) = args.message.clone() {
            ...
            logs = logs.into_iter().filter(|log| re.is_match(&log.message)).collect()
        }

        if let Some(before) = args.before.clone() {
            let date = NaiveDateTime::parse_from_str(&before,&args.date_fmt).unwrap();
            logs = logs.into_iter().filter(|log| log.time.and_utc().timestamp() <= date.and_utc().timestamp()).collect();
        }
        ...
        for log in logs {
            ...
            println!("{}",strfmt::strfmt(args.fmt.as_str(), &vars).expect("Failed to format output"))
        }
